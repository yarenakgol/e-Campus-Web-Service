...
public class UserUpdateDTO {
	...
	@Size(min=1, max=35, message ="{backend.constraints.email.NotNull.message}")
	private  String eMail;

	@NotNull(message = "{backend.constraints.phoneNum.Size.message}")
	@Size(min=5, max=15, message ="{backend.constraints.phoneNum.NotNull.message}")
	private  String phoneNum;

	@NotNull(message = "{backend.constraints.ogrNo.Size.message}")
	@Size(min=5, max=15, message ="{backend.constraints.ogrNo.NotNull.message}")
